...
export interface SelectFieldProps {
  ...
  className?: string;
  setSuggestionsFromApi: (props) => void;
  loading: boolean;
  getInputLabel: (option) => string;
}
...
const SelectField = ({
  ...
  const handleBlur = (event) => {
    if (!event.currentTarget.contains(event.relatedTarget)) {
      ...
      if (!value) {
        ...
        setInputValue(getInputLabel(value));
      }
    }
  };
  ...
  const handleClick = (option) => {
    setShowSelect(false);
    setInputValue(getInputLabel(option));
    onChange(option);
  };
  ...
  const handleOnChangeInput = (input) => {
    ...
    if (inputValue.length > 0) {
      setShowSelect(true);
      setSuggestionsFromApi(inputValue);
    } else {
      setShowSelect(false);
      onChange(null);
    }
